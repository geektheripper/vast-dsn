...
func Load(dsn string) (opts *S3Options, bucket string, key string, err error) {
	opts = &S3Options{
		Protocol: "https",
		Region:   "us-east-1",
...
func MustLoad(dsn string, logger ...utils.Logger) (opts *S3Options, bucket string, key string) {
	log := utils.EnsureLogger(logger...)

	opts, bucket, key, err := Load(dsn)
	if err != nil {
...
func NewS3(dsn string) (opts *S3Options, err error) {
	opts, bucket, _, err := Load(dsn)
	if bucket != "" {
...
func NewS3Bucket(dsn string) (opts *S3Options, bucket string, err error) {
	opts, bucket, key, err := Load(dsn)
...
func MustNewS3Bucket(dsn string, logger ...utils.Logger) (*S3Options, string) {
	log := utils.EnsureLogger(logger...)

	s3dsn, bucket, err := NewS3Bucket(dsn)
	if err != nil {
...
func NewS3Path(dsn string) (opts *S3Options, bucket string, path string, err error) {
	opts, bucket, key, err := Load(dsn)
	if bucket == "" {
...
func MustNewS3Path(dsn string, logger ...utils.Logger) (*S3Options, string, string) {
	log := utils.EnsureLogger(logger...)

	s3dsn, bucket, path, err := NewS3Path(dsn)
	if err != nil {
...
func NewS3Object(dsn string) (opts *S3Options, bucket string, key string, err error) {
	opts, bucket, key, err = Load(dsn)
	if bucket == "" {
...
func MustNewS3Object(dsn string, logger ...utils.Logger) (*S3Options, string, string) {
	log := utils.EnsureLogger(logger...)

	s3dsn, bucket, key, err := NewS3Object(dsn)
	if err != nil {
